...
### 4. O runner auto-hospedado, e o alerta que a disciplina faz

**Por que um runner auto-hospedado:** o cluster roda na máquina de quem o provisionou (ADR-024), e o runner hospedado pelo GitHub não alcança essa máquina. A aula de GitHub Actions em ambientes on-premises apresenta o runner auto-hospedado justamente para jobs que precisam de recursos dentro da rede local.
...
3. o runner tem rótulo próprio, `oficina-local`, e roda em primeiro plano só nas janelas de trabalho, nunca como serviço;
...
5. nenhum segredo fica gravado no runner, que é uma das boas práticas da aula de on-premises: o login no registro usa o token do job e é desfeito ao fim dele, o checkout não guarda credencial, e o kubeconfig vive no diretório temporário do job.

**Com o runner desligado,** o deploy espera na fila e os outros dois jobs rodam normalmente. A aula de on-premises registra o limite: um job para runner auto-hospedado fica no máximo 24 horas na fila antes de falhar.

### 5. O banco de dados na pipeline

O enunciado pede o banco provisionado pelo Terraform e, na pipeline, o deploy do banco de dados. **A leitura adotada:** o Terraform **provisiona** o banco, e a pipeline **não roda `terraform apply`**. O estado do Terraform é local (ADR-024), e um `apply` disparado pelo runner teria um estado diferente do `apply` feito no terminal da mesma máquina, e tentaria criar um cluster que já existe. O deploy do banco de dados, na pipeline, é o passo com esse nome, que confirma o banco provisionado de pé, mostra no log o que o Flyway fez no boot da aplicação, a migration aplicada ou o aviso de que o schema já está em dia, e espera a carga de demonstração terminar.
