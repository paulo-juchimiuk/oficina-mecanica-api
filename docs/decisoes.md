...
**Decisão:** o cluster Kubernetes desta fase é **local**, criado com `kind` e provisionado pelo Terraform, e o **banco de dados é provisionado pelo mesmo `apply`, dentro do cluster**. Os providers são `tehcyx/kind`, para o cluster, e `hashicorp/kubernetes`, para o namespace, o segredo, o volume, o banco e o serviço dele. **Não se usa `helm`.** Os objetos da aplicação são declarados em manifestos YAML em `/k8s` e aplicados com `kubectl`, fora do Terraform. Nenhum recurso é criado em provedor de nuvem.
...
**Fundamento técnico:** `kind` executa um cluster Kubernetes conforme dentro de contêineres Docker, e é mantido pelo próprio SIG de testes do Kubernetes, o que o torna a implementação local mais próxima de um cluster real. O `metrics-server` roda nele, e sem métrica de CPU e memória o Horizontal Pod Autoscaler não escala, o que tornaria a demonstração de escalabilidade impossível. O Terraform gerencia o cluster pelo provider do `kind` e o banco por recursos tipados do provider `kubernetes`, de modo que o mesmo `apply` que existe contra a nuvem existe aqui. Três detalhes do ambiente decorrem disso e estão documentados em `infra/README.md`: a versão do Kubernetes é a que o provider embute, porque ele não chama o CLI do `kind` e sim a biblioteca dele, e por isso a imagem do nó é fixada por digest; o volume do banco é declarado sem esperar ligação, porque a StorageClass do cluster só liga o volume quando um Pod o monta, e o Pod que o monta nasce depois; e o banco é provado de pé pelo `rollout` do Deployment, que o `apply` acompanha até o fim.
...
**Onde fica a fronteira entre Terraform e `kubectl`, e por que ela não é arbitrária:** o Terraform cria o que precisa existir **antes** da aplicação, ou seja, o cluster, o namespace e o banco, e é isso que o enunciado pede dele. Os objetos da aplicação ficam em `/k8s` como YAML porque é o que o enunciado pede em outro item, e porque é o que a pipeline aplica a cada publicação. Declarar os mesmos objetos nos dois lugares criaria dois donos para o mesmo fato e uma disputa a cada `apply`. **`helm` fica fora por três razões somadas:** o enunciado pede manifestos, não gráficos; a disciplina de Kubernetes Avançado usa Helm para empacotar a própria aplicação, que é problema de distribuição e não de provisionamento; e um pacote a mais entre o YAML e o cluster esconde justamente o que o vídeo precisa mostrar.

**O anti-padrão que a apostila nomeia, e a resposta:** a apostila de StatefulSets chama de *"a falácia do deployment para bancos de dados"* rodar banco em Deployment, e os três problemas que ela lista partem todos da mesma premissa, *"se você usar um Deployment com replicas: 3"*: o disco único disputado por três Pods, os nomes aleatórios que impedem o quórum, e a colisão de eleições somada ao desligamento que manda o sinal de parada aos três ao mesmo tempo, sem deixar o banco drenar para o disco. **Nenhum dos três existe com uma réplica e estratégia `Recreate`:** há um Pod por vez, montando sempre o mesmo volume, sem vizinho para reconhecer, sem líder para eleger, e recebendo sozinho o sinal de parada, com o período de carência do próprio Pod para encerrar. O StatefulSet resolve identidade estável e volume por réplica, e este ambiente não tem réplicas para distinguir. Se o banco desta aplicação algum dia precisar de réplica, a decisão muda com ela.

**Alternativa recusada:** o banco como contêiner Docker ao lado do cluster, pelo provider `kreuzwerker/docker`. Tiraria o volume do caminho, mas a pipeline publicaria a aplicação num cluster que não contém o banco dela, o vídeo teria dois mundos para explicar, e a persistência dentro do cluster é exatamente o que a disciplina de Kubernetes ensina.

**Consequência aceita:** o executor hospedado do provedor de CI não alcança um cluster que roda na máquina do desenvolvedor, então a etapa de publicação do pipeline precisa de um executor com acesso ao cluster. O estado do Terraform é local, então a pipeline não executa `apply`: ela encontra o ambiente já provisionado. E o desenho da arquitetura entregue descreve um ambiente local, não um ambiente de produção em nuvem, o que precisa estar dito no README para que documento e ambiente não se contradigam.
...
## ADR-027: Publicação no Kubernetes por manifestos e pipeline, com escala pela CPU

**Decisão:** a aplicação é publicada no cluster do ADR-024 por manifestos YAML em `/k8s`, aplicados com `kubectl`: Deployment, Service, ConfigMap, Secret e HorizontalPodAutoscaler da aplicação, a caixa de e-mail do ambiente, o Job da carga de demonstração e o `metrics-server`. O HPA escala a aplicação de 1 a 4 réplicas **só pela CPU**, com alvo de 70% do pedido de cada réplica. A publicação é feita por uma pipeline do GitHub Actions, em `.github/workflows/ci-cd.yml`, com três jobs: build e testes e a imagem no executor hospedado do GitHub, e o deploy num **executor auto-hospedado** na máquina onde o cluster roda. A imagem vai para o GitHub Container Registry. A pipeline não provisiona nada: o Terraform provisiona o cluster e o banco, e a pipeline confirma o banco, publica a aplicação e carrega os dados de demonstração.

**Fundamento de negócio:** o uso do sistema varia ao longo do dia, e a capacidade precisa acompanhar essa variação sem alguém ajustando réplicas à mão: ela sobe quando a demanda sobe e é devolvida quando a demanda cai. E cada mudança no código precisa chegar ao ambiente pelo mesmo caminho, construída e testada antes, sem depender de alguém lembrar a sequência de comandos.

**Fundamento técnico:** cada escolha dos manifestos responde a um comportamento do Kubernetes reproduzido neste cluster, a escolha da métrica do HPA responde a uma conta feita com o uso medido das réplicas, e a forma da pipeline responde a onde o cluster roda. As seções abaixo trazem as três.
...
- **Três sondas HTTP, como a aula de probes da disciplina de Kubernetes ensina:** inicialização, prontidão e vivacidade. A aula recomenda um endpoint para cada tipo de sonda, para evitar conflito entre elas. Prontidão e vivacidade seguem a recomendação, com `/actuator/health/readiness` e `/actuator/health/liveness`, e usam os intervalos do exemplo da aula: prontidão a cada 5 segundos com espera de 3, vivacidade a cada 10 com espera de 5. A sonda de inicialização se afasta da recomendação e repete o endpoint da vivacidade, porque ela existe para segurar a vivacidade até o boot terminar e pergunta a mesma coisa com mais paciência: a cada 5 segundos, por até 3 minutos. O exemplo da aula espera 120 segundos antes da primeira consulta; aqui o boot leva cerca de 16 segundos, e uma espera fixa de 2 minutos deixaria cada réplica nova fora do Service justamente na janela em que o HPA precisa dela.
...
- **Cada réplica espera 10 segundos antes de começar a desligar**, por um `preStop` com `sleep`. Quando uma réplica sai, pelo rollout ou pela descida do HPA, a retirada dela do Service e o desligamento da aplicação correm ao mesmo tempo, e por um instante chamadas ainda chegam a uma réplica que já parou de aceitar conexão. Medido neste cluster, com duas réplicas e chamadas contínuas à porta 8080 durante um rollout: sem a pausa, 3 de 405 chamadas falharam, uma delas presa até o tempo limite; com a pausa, nenhuma de 418. É o ajuste que a documentação do Spring Boot recomenda para rodar no Kubernetes, e é ele que sustenta a frase de cima, de que a atualização não perde capacidade.
...
- **Todo objeto da aplicação declara o namespace `oficina`.** O `kubectl apply -f k8s/` aplica objetos de dois namespaces, porque o `metrics-server` vive em `kube-system`, e por isso não pode receber `-n`; e o endereço curto do banco, `banco:5432`, só resolve de dentro do namespace dele, pelo motivo registrado em `infra/README.md`.
...
**A carga que prova a escala** é um Pod dentro do cluster chamando o login em laço. O login confere a senha com BCrypt, que custa CPU a cada chamada, e isso simula o aumento de carga que o enunciado pede sem instalar ferramenta; a aula cita K6 e JMeter, que fariam o mesmo com uma instalação a mais. Medido neste cluster: a CPU passou do alvo cerca de 30 segundos depois de a carga começar, e o HPA chegou a 4 réplicas em menos de um minuto, numa medição passando por 3 e nas outras indo direto de 1 para 4. As duas sequências são a mesma conta: a primeira leitura acima do alvo dá `teto(1 × uso ÷ 70)`, que já é 4 quando o uso passa de 210%. Com a carga desligada, a CPU caiu abaixo do alvo em pouco mais de um minuto, e o HPA voltou a 1 réplica cerca de 6 minutos depois de a carga parar, dos quais 5 são a janela de estabilização padrão da descida.

### 3. A pipeline

- **Um arquivo só, `.github/workflows/ci-cd.yml`**, o nome que a aula de fundamentos da disciplina de GitHub Actions usa. Ela dispara em `push` e `pull_request` na `main` e pelo botão de execução manual da aba Actions.
- **Build e testes, no executor hospedado:** o build da aplicação e o `mvn verify`, com os testes de integração em Testcontainers, porque o executor hospedado tem Docker. No mesmo job, `terraform init -backend=false`, `fmt -check` e `validate`, comandos da aula de principais comandos da disciplina de Terraform, que conferem o código sem precisar do estado. A aula de IaC em pipelines da disciplina de DevOps põe o `plan` na pipeline como ensaio antes de aplicar; aqui ele fica fora, porque o estado é local (ADR-024) e o executor hospedado não o tem, então um `plan` lá anunciaria criar tudo de novo a cada execução. O `validate` sem `init` falha por não encontrar os providers, e por isso o `init` sem backend vem antes.
- **A imagem, no executor hospedado, só na `main` e fora de pull request:** publicada no GitHub Container Registry com o token do próprio job, com a tag do SHA do commit, que é imutável, e a tag `main`. A disciplina de GitHub Actions ensina o Docker Hub com um token pessoal guardado em segredo; o registro do GitHub faz o mesmo sem conta nova e sem segredo a rotacionar.
- **O deploy, no executor auto-hospedado, só na `main` e fora de pull request.** A tag do SHA substitui `main` numa cópia dos manifestos, e a troca é conferida na cópia antes de qualquer comando no cluster, então cada commit publicado vira exatamente uma revisão do Deployment, apontando para a imagem dele; aplicar o manifesto com `main` e trocar a imagem depois geraria duas revisões por publicação, e publicar de novo o mesmo commit não cria revisão, porque o Deployment não muda. O manifesto versionado continua com `main`, para que `kubectl apply -f k8s/` funcione à mão, e o job carrega a imagem nos nós com as duas tags.
- **O job gera o próprio kubeconfig** com `kind get kubeconfig`, no diretório temporário do job, e não usa o contexto corrente do `kubectl` da máquina: se a máquina conhece outros clusters, a publicação não tem como cair num deles.
- **O passo "Deploy do banco de dados" vem depois da aplicação dos manifestos**, porque o schema nasce no boot da aplicação, pelo Flyway, e a carga espera o schema existir.

### 4. O executor auto-hospedado, e o alerta que a disciplina faz

**Por que um executor auto-hospedado:** o cluster roda na máquina de quem o provisionou (ADR-024), e o executor hospedado do GitHub não alcança essa máquina. A aula de GitHub Actions em ambientes on-premises apresenta o executor auto-hospedado justamente para jobs que precisam de recursos dentro da rede local.

**O alerta:** a aula de otimização de pipelines da disciplina de DevOps diz que *"self-hosted runners não são recomendados para repositórios públicos, pois o código de qualquer pull request pode ser executado em sua infraestrutura"*. Este repositório é público, e a resposta são cinco medidas somadas:

1. o repositório exige aprovação para rodar workflow de qualquer colaborador externo, e é essa a medida que de fato segura, porque num pull request o workflow executado é o do branch do próprio pull request;
2. o job de deploy só roda em `push` na `main` e pelo botão de execução manual;
3. o executor tem rótulo próprio, `oficina-local`, e roda em primeiro plano só nas janelas de trabalho, nunca como serviço;
4. o workflow declara a permissão mínima: leitura do conteúdo, escrita de pacote só no job da imagem e leitura de pacote no deploy;
5. nenhum segredo fica gravado no executor, que é uma das boas práticas da aula de on-premises: o login no registro usa o token do job e é desfeito ao fim dele, o checkout não guarda credencial, e o kubeconfig vive no diretório temporário do job.

**Com o executor desligado,** o deploy espera na fila e os outros dois jobs rodam normalmente. A aula de on-premises registra o limite: um job para executor auto-hospedado fica no máximo 24 horas na fila antes de falhar.

### 5. O banco de dados na pipeline

O enunciado pede o banco provisionado pelo Terraform e, na pipeline, o deploy do banco de dados. **A leitura adotada:** o Terraform **provisiona** o banco, e a pipeline **não roda `terraform apply`**. O estado do Terraform é local (ADR-024), e um `apply` disparado pelo executor teria um estado diferente do `apply` feito no terminal da mesma máquina, e tentaria criar um cluster que já existe. O deploy do banco de dados, na pipeline, é o passo com esse nome, que confirma o banco provisionado de pé, mostra no log o que o Flyway fez no boot da aplicação, a migration aplicada ou o aviso de que o schema já está em dia, e espera a carga de demonstração terminar.

**Porquê:** é a configuração que sobe e desce sozinha com a demanda desta aplicação, com os objetos que o enunciado nomeia e mais os três que o ambiente precisa para funcionar: a caixa de e-mail, a carga de demonstração e o coletor de métricas. E a pipeline publica no mesmo cluster em que o Terraform provisionou o banco e em que o HPA escala, então publicação, banco e escala são um ambiente só.
