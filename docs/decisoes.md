...
**O anti-padrão que a apostila nomeia, e a resposta:** a apostila de StatefulSets chama de *"a falácia do deployment para bancos de dados"* rodar banco em Deployment, e os três problemas que ela lista partem todos da mesma premissa, *"se você usar um Deployment com replicas: 3"*: o disco único disputado por três Pods, os nomes aleatórios que impedem o quórum, e a eleição simultânea de líder. **Nenhum dos três existe com uma réplica e estratégia `Recreate`:** há um Pod por vez, montando sempre o mesmo volume, sem vizinho para reconhecer e sem líder para eleger. O StatefulSet resolve identidade estável e volume por réplica, e este ambiente não tem réplicas para distinguir. Se o banco desta aplicação algum dia precisar de réplica, a decisão muda com ela.
