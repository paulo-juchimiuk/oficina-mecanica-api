...
- **Build e testes, no executor hospedado:** o build da aplicação e o `mvn verify`, com os testes de integração em Testcontainers, porque o executor hospedado tem Docker. No mesmo job, `terraform init -backend=false`, `fmt -check` e `validate`, que é a conferência do Terraform antes de aplicar que a aula de IaC em pipelines da disciplina de DevOps ensina, feita sem o estado, que é local (ADR-024). O `validate` sem `init` falha por não encontrar os providers, e por isso o `init` sem backend vem antes.
...
- **O deploy, no executor auto-hospedado, só na `main` e fora de pull request.** A tag do SHA substitui `main` numa cópia dos manifestos na hora de aplicar, então cada publicação vira exatamente uma revisão do Deployment, apontando para a imagem do commit; aplicar o manifesto com `main` e trocar a imagem depois geraria duas revisões por publicação. O manifesto versionado continua com `main`, para que `kubectl apply -f k8s/` funcione à mão, e o job carrega a imagem nos nós com as duas tags.
